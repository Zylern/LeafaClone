...
def check_owner(func):
    async def wrapper(_, CallbackQuery):
        ...
        gid = CallbackQuery.message.chat.id
        if [gid, msg_id] in user_search[user]:
            try:
                await func(_, CallbackQuery)
            except FloodWait as e:
                await asyncio.sleep(e.x)
            except MessageNotModified:
                pass
        else:
            await CallbackQuery.answer("Not your query!!!", show_alert=True)
    return wrapper
...
async def iytdl_inline(client, message):
...
@check_owner
async def ytdl_download_callback(client, CallbackQuery):
    yt_code = CallbackQuery.matches[0].group(1)
    choice_id = CallbackQuery.matches[0].group(2)
    downtype = CallbackQuery.matches[0].group(3)
    if str(choice_id).isdigit():
        choice_id = int(choice_id)
        if choice_id == 0:
            await CallbackQuery.answer("🔄  Processing...", show_alert=False)
    ...
    await CallbackQuery.answer(callback_continue, show_alert=True)
    ...
    if type(k)==list:
        await CallbackQuery.answer(k[1], show_alert=True)
        return
    _fpath = ""
    thumb_pic = None
...
@check_owner
async def ytdl_callback(_, CallbackQuery):
    choosen_btn = CallbackQuery.matches[0].group(1)
    data_key = CallbackQuery.matches[0].group(2)
    page = CallbackQuery.matches[0].group(3)
    ...
    if choosen_btn == "back":
        ...
        index = int(page) + 1
        if index > total:
            return await CallbackQuery.answer("That's All Folks !", show_alert=True)
        await CallbackQuery.answer()
        ...
        await CallbackQuery.answer("View Changed to:  📜  List", show_alert=False)
        list_res = ""
        ...
        index = 1
        await CallbackQuery.answer("View Changed to:  📰  Detailed", show_alert=False)
